...
tree = ET.parse(r'e:\school\mypaper\unpacked\word\document.xml')
root = tree.getroot()
...
rels_tree = ET.parse(r'e:\school\mypaper\unpacked\word\_rels\document.xml.rels')
...
def extract_math_text(elem):
    """Extract readable text from math elements"""
    parts = []
    for node in elem.iter():
        tag = node.tag.split('}')[-1] if '}' in node.tag else node.tag
        if tag == 't':  # m:t - math text
            if node.text:
...
output = '\n'.join(lines)
with open(r'e:\school\mypaper\doc_content_full.txt', 'w', encoding='utf-8') as f:
    f.write(output)
